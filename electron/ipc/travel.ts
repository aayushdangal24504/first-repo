...
type TripDayRow = { id: string; tripId: string; dayDate: string; title: string | null; entryJson: string; expensesJson: string; packingJson: string; createdAt: string; updatedAt: string };
...
const daySelect = `SELECT id, trip_id as tripId, day_date as dayDate, title, entry_json as entryJson, expenses_json as expensesJson, packing_json as packingJson, created_at as createdAt, updated_at as updatedAt FROM trip_days`;
...
const mapTripDay = (row: TripDayRow): TripDay => ({ id: row.id, tripId: row.tripId, dayDate: row.dayDate, title: row.title, entry: safeJson<{ text: string }>(row.entryJson, { text: '' }), expenses: safeJson(row.expensesJson, []), packing: safeJson<string[]>(row.packingJson, []), createdAt: row.createdAt, updatedAt: row.updatedAt });
...
export const registerTravelIpc = (ipcMain: IpcMain, db: AppDatabase, auth: AuthService): void => {
  ...
  ipcMain.handle('travel:delete', async (_event, id: string): Promise<{ ok: true }> => { db.prepare('DELETE FROM trips WHERE id = ? AND user_id = ?').run(id, auth.getCurrentUserId()); return { ok: true }; });
  ipcMain.handle('travel:create-day', async (_event, input: CreateTripDayInput): Promise<TripDay> => {
    ...
    db.prepare(`INSERT INTO trip_days (id, user_id, trip_id, day_date, title, entry_json, expenses_json, packing_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, userId, input.tripId, dayDate, input.title?.trim() || null, JSON.stringify({ text: input.text?.trim() || '' }), '[]', JSON.stringify(cleanList(input.packing)), now, now);
  ...
  ipcMain.handle('travel:delete-day', async (_event, id: string): Promise<{ ok: true }> => {
    db.prepare('DELETE FROM trip_days WHERE id = ? AND user_id = ?').run(id, auth.getCurrentUserId());
    return { ok: true };
  });
  ...
  ipcMain.handle('travel:update-day', async (_event, input: UpdateTripDayInput): Promise<TripDay> => {
    ...
    db.prepare(`UPDATE trip_days SET day_date = ?, title = ?, entry_json = ?, expenses_json = ?, packing_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?`).run(input.dayDate === undefined ? current.dayDate : cleanDate(input.dayDate) ?? current.dayDate, input.title === undefined ? current.title : input.title?.trim() || null, JSON.stringify({ text: input.text === undefined ? current.entry.text : input.text }), JSON.stringify(input.expenses === undefined ? current.expenses : input.expenses), JSON.stringify(input.packing === undefined ? current.packing : cleanList(input.packing)), input.id, userId);
